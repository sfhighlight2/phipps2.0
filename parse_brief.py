...
blocks = re.split(r'### \d{2} — ', md)[1:] # split by service starting at 01
...
for block in blocks:
    ...
    print(f"Parsed {service_name}: {file_name}, Cross:{cross_links}")
    
    # Generate Cross Links HTML
    cross_html = ""
    ...
    page_html = f'''
    <!-- ═══ HERO ═══ -->
    <section class="overlay-hero" style="height:60vh; min-height:400px;">
      <img alt="{service_name}" class="hero-bg" src="images/{hero_img}" style="filter:brightness(0.6);" />
      <div class="container hero-container" style="justify-content:center; align-items:center; text-align:center;">
        <h1 class="reveal d1" style="font-size:52px; font-weight:400;">{title}</h1>
        <p class="reveal d2" style="font-size:18px; max-width:800px;">{tagline}</p>
      </div>
    </section>
    ...
    # Extract everything from end of nav to start of footer
    nav_end = existing.find('</nav>')
    if nav_end != -1:
        # Also skip the mobile menu which follows </nav>
        mobile_end = existing.find('</div>', nav_end)  # approx
        # A much safer bet is to find a specific string or just use regex
        body_start = existing.find('<!-- ═══ HERO ═══ -->')
        if body_start == -1: 
            body_start = existing.find('<section class="overlay-hero">')
        if body_start == -1:
            body_start = existing.find('<div class="hero')
